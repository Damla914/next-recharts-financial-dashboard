...
import { deleteBankRatios } from "@/lib/actions/bankRatio";
...
export default function DeleteRatioModal({item}: {item: DeleteModalProps}) {
    ...
    const handleDelete = async () => {
        setLoading(true);
        const res = await deleteBankRatios(item.id);
        setLoading(false);
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4 text-center">
                ...
                <div>
                   ...
                   <p className="text-xs text-slate-500 mt-1">
                        Are you sure you want to delete <span className="font-semibold text-slate-700">({item.year})</span>? This action cannot be undone.
                    </p>
                </div>
